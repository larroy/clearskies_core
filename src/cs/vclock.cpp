...
bool Vclock::is_descendant(const Vclock& parent) const
{
    ...
    for (const auto& key: keys)
    {
        ...
        i64 delta_dp = desc_val - paren_val;
        if (delta_dp >= numeric_limits<u32>::max() / 2)
            return false;

        if (delta_dp <= - (i64) (numeric_limits<u32>::max() / 2))
            return false;
...
u32 Vclock::operator[](const std::string& key) const
{
